...
# Optional Parameters
Optional parameters can be passed as a `params::Dict{String,<:Any}`. Valid keys are:
- `"IndicatorType"`: See Detective investigations..
- `"MaxResults"`: List the maximum number of indicators in a page.
...
# Optional Parameters
Optional parameters can be passed as a `params::Dict{String,<:Any}`. Valid keys are:
- `"FilterCriteria"`: Filter the investigation results based on a criteria.
- `"MaxResults"`: List the maximum number of investigations in a page.
- `"NextToken"`: List if there are more results available. The value of nextToken is a
  unique pagination token for each page. Repeat the call using the returned token to retrieve
  the next page. Keep all other arguments unchanged. Each pagination token expires after 24
  hours. Using an expired pagination token will return a Validation Exception error.
...
- `graph_arn`: The ARN of the behavior graph.
- `scope_end_time`: The data and time when the investigation began. The value is an UTC
