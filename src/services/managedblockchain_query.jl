...
every token in the request.  Only the native tokens BTC,ETH, and the ERC-20, ERC-721, and
...
or contract) on the blockchain.  Only the native tokens BTC,ETH, and the ERC-20, ERC-721,
...
"""
    get_transaction(network, transaction_hash)
    get_transaction(network, transaction_hash, params::Dict{String,<:Any})

Get the details of a transaction.

# Arguments
- `network`: The blockchain network where the transaction occurred.
- `transaction_hash`: The hash of the transaction. It is generated whenever a transaction
  is verified and added to the blockchain.

"""
get_transaction(
    network, transactionHash; aws_config::AbstractAWSConfig=global_aws_config()
) = managedblockchain_query(
    "POST",
    "/get-transaction",
    Dict{String,Any}("network" => network, "transactionHash" => transactionHash);
    aws_config=aws_config,
    feature_set=SERVICE_FEATURE_SET,
)
    ...
    return managedblockchain_query(
        ...
        Dict{String,Any}(
            mergewith(
                _merge,
                Dict{String,Any}(
                    "network" => network, "transactionHash" => transactionHash
                ),
                params,
            ),
        );
...
"""
    list_transaction_events(network, transaction_hash)
    list_transaction_events(network, transaction_hash, params::Dict{String,<:Any})
...
# Optional Parameters
Optional parameters can be passed as a `params::Dict{String,<:Any}`. Valid keys are:
- `"maxResults"`: The maximum number of transaction events to list.  Even if additional
  results can be retrieved, the request can return less results than maxResults or an empty
  array of results. To retrieve the next set of results, make another request with the
  returned nextToken value. The value of nextToken is null when there are no more results to
  return
- `"nextToken"`: The pagination token that indicates the next set of results to retrieve.
"""
...
- `"maxResults"`: The maximum number of transactions to list.  Even if additional results
  can be retrieved, the request can return less results than maxResults or an empty array of
  results. To retrieve the next set of results, make another request with the returned
  nextToken value. The value of nextToken is null when there are no more results to return
- `"nextToken"`: The pagination token that indicates the next set of results to retrieve.
- `"sort"`: Sorts items in an ascending order if the first page starts at fromTime. Sorts
